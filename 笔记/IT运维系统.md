...
![image-20220316164807829](images/image-20220316164807829.png)- 
...
![image-20220316165126226](images/image-20220316165126226.png)

##### 上图为模型ID为10001（Linux服务器）下的部分资源

- #### rlt_motype_obj 模型-资源关联表

![模型-资源关系表](IT运维/模型-资源关系表.png)
...
以Linux服务器为例 该模型有两个指标集 ICMP指标集和SNMP指标集 对应的数据表为  **md_indicatorset**
...
- ##### INDICATORTABLE：指标数据所对应的表

指标所在的表为**md_indicator**

![指标表](IT运维/指标表.png)

字段：

![指标表字段](IT运维/指标表字段.png)
...
- COLUMNNAME：对应列名（KPI__XX,即该指标在指标采集表中所在的列）
...
- pageSize：分页大小
- curPage：当前页面
- alarmId：告警状态ID（默认-1） 关联**rlt_obj_alarmstatus**表
- venderId：销售公司ID（默认-1）关联**cfg_vendor**表
- domainId：所属域ID（默认-1）关联**cfg_domain**表
- motypeId：模型ID（默认-1）
- searching：搜索内容（默认”“）
...
```java
List<String> objIds = commonServiceAPI.getResCodeList(Common.getRemoteUser(request));
```
...
- user_hasNodes 根据用户ID 获取其有权限的资源ID
  ...
  - 1、根据用户ID查询用户角色，根据**pms_role_node**表的关联查找具体资源
  - 2、根据用户ID查询用户角色，根据**pms_role_motype**表找到相应的模型ID,再根据**rlt_motype_obj**找到模型下的资源
  - 3、两者取并集
...
```java
moinfos = resourceCount.getAllEntitesForCount(pageBounds, objIds, params, searching, alarmId, venderId,
        motypeId, domainId);
```
...
<foreach collection="params" item="item" index="key" separator="union">
        SELECT
        O.OBJID,O.OBJNAME,O.OBJCODE,O.IP,O.MOTYPEID,case when O.VENDORID is null then 0 else O.VENDORID end as
        VENDORID,O.MODEL,V.VENDORNAME,O.DOMAIN AS DOMAINID,D.DOMAINNAME,
        '${item.MOTYPETABLE}' as MOTYPETABLE,
        '${item.MOTYPENAME}' as MOTYPENAME,
        '${item.MOTYPECODE}' as MOTYPECODE
        FROM
        ${item.MOTYPETABLE} O LEFT JOIN cfg_vendor V ON O.VENDORID=V.VENDORID LEFT JOIN cfg_domain D ON
        O.DOMAIN=D.DOMAINID
</foreach>
```
...
```sql
<select id="getAllEntitesForCount" resultType="java.util.HashMap" parameterType="java.util.Map" databaseId="mysql">
    SELECT
    ...
    LEFT JOIN rlt_obj_alarmstatus OA ON A.OBJID = OA.OBJID LEFT JOIN cfg_alarmlevel L ON
 ...
 for (Map<String, Object> info : moinfos)
            {
                ...
                List<Map<String, Object>> subCounts = resourceCount.getSubTypeCountOfEntity(objId);
                info.put("subCounts", subCounts);
            }
```

根据**rlt_motype_obj** 表中父ID和父类型来分组计数
...
在模型表md_moType中新增一条数据，插入时根据获取到的模型ID，生成相应表名**obj_${id}**

```sql
 <selectKey keyProperty="id" resultType="java.lang.Integer" order="BEFORE" databaseId="oracle">
        select seq_obj_id.nextval id from dual
 </selectKey>
 <selectKey keyProperty="id" resultType="java.lang.Integer" order="BEFORE" databaseId="mysql">
        select nextval('SEQ_OBJ_ID')
 </selectKey>
 
     insert into Md_Motype (motypeid,motypecode,motypename,ISTOP,motypetable,createuser,createtime)
     values (#{id}, #{moTypeCode},#{moTypeName},#{isTop},'obj_${id}',#{createUser},#{createtime})
```
...
#### 3、如果该模型有父类型 parentIds不为空 增加两者之间的关联（addMoRelation(parentIds, moTypeCode, request);）

在**rlt_motype_motype**表中插入数据
...
在**rlt_commonmotype_motype**中插入数据
...
{
    map.put("attributeUnitl", attributeJson.get("unit"));
}
```
...
```sql
select replace(t.columnname,'ATT_','') as NUM  from md_moattribute t where t.motypeid=#{_parameter}
```

- 在**md_moattribute**表插入数据
...
if (!"*".equals(attributeJson.get("comAttrName")))
{
    ...
    Map<String, Object> commonAttributeMap = modelTypeDaoAPI.getCommonAttributeMap(map);
    String commonAttributeId = String.valueOf(commonAttributeMap.get("COMMONATTRIBUTEID"));
    // 判断属性是否存在
...
commonServiceAPI.sendMessageToProbe(ProbeType.Performance, ProbeCommandName.update);
```

```java
//枚举类
public enum ProbeType {
    Discover, Configuration, Performance, Dial, Event, Inspect, ConfigFile, Process,vmware
}
...
![采集凭证配置](IT运维/采集凭证配置.png)

**collect_credential **采集凭证表

![采集凭证表](IT运维/采集凭证表.png)

param与新增时参数对应 **PROTOCOLID**  为协议ID

![snmp采集凭证参数](IT运维/snmp采集凭证参数.png)
...
JobDetail jobDetail = JobBuilder
    				  ...
    				  .withIdentity("collectTask", Scheduler.DEFAULT_GROUP)  //分组标识
                      .build();
```
...
每过5分钟后就会执行**TaskManager.getInstance().executeTask()**方法
...
Date startTime = new Date(nowTime - nowTime % (taskPeriod * 1000));  //格式化时间 去除偏差 精确到 00秒
```
...
List<ObjectModel> objectModelList = objectManager.getObjectList(
         ProbeContextHolder.getStringValue(Constants.PROBE_PROTOCOL), System.getProperty(Constants.PROBE_ID));
```
...
- protocol  探针采集协议此处值为（“SNMP”）
  ...
  - **Constants.PROBE_PROTOCO** 常量 ProbeProtocol
...
![根据探针ID获取各模型对应协议下的指标集](IT运维/根据探针ID获取各模型对应协议下的指标集.png)
...
WHERE PROTOCOLNAME = ?) 
AND OBJ.OBJID IN
( SELECT MO.OBJID FROM rlt_motype_obj MO , collect_probe P WHERE MO.SERVERID = P.SERVERID AND P.PROBEID = ?)
```
...
![资源-采集凭证](IT运维/资源-采集凭证.png)

根据asstype 对allObjList进行筛选 得到筛选后的数组objList

- 0：全部
- 1：动态
- 2：静态

遍历allObjList，以及凭证参数键值对，取出

- credentialId  凭证ID
- credentialName 凭证名称
- ip 资源IP

凭证参数进行AES128接码后的键值对，以下为snmp凭证参数（不同协议参数不同）：

- port 端口
- snmpVersion snmp协议版本
- community 认证需要的community 

将资源属性、采集指标集和上述的采集凭证信息封装到objectModel对象中：

```java
if (ObjectUtils.isEmpty(ObjectModelMap.get(objid)))
                {
                    ObjectModel objectModel = new ObjectModel();
                    objectModel.setObjId(objid);
                    objectModel.setObjectName(objname);
                    objectModel.setIp(ip);
                    objectModel.setCredential(inMap);
                    objectModel.setMoTypeId(motypeId);
                    objectModel.setIndicatorSetIdList(new ArrayList<>());
                    if (objList.contains(objectMap))
                    {
                        objectModel.getIndicatorSetIdList().add(indicatorsetid);
                    }
                    ObjectModelMap.put(objid, objectModel);
                }
                else
                {
                    if (objList.contains(objectMap))
                    {
                        ObjectModel objectModel = ObjectModelMap.get(objid);
                        objectModel.getIndicatorSetIdList().add(indicatorsetid);
                        ObjectModelMap.put(objid, objectModel);
                    }
                }
...
![ObjectModel数据样例](IT运维/ObjectModel数据样例.png)
...
```java
 IndicatorSetModel indicatorSetModel = IndicatorsManager.indicatorSetModelMap.get(indicatorSetId);
...
遍历采集对象，开启一个新的线程放入线程池中来进行采集任务

```java
ListenableFuture<FutureModel> future = 
    ThreadPoolManager.getInstance().submit(new TaskThread(rootObjectModel, taskModel.getStartTime()));
```
...
```java
ResultObjectModel resultObjectModel = 
    getResultObjectModel(indicatorSetId, rootObjectModel.getObjId(),
                        null, collect, isConnect, starTime, valuesMap, logMap);
```

##### getResultObjectModel

```java
private ResultObjectModel getResultObjectModel(String indicatorSetId, String objId, String index,
            ICollector collect, boolean isConnect, Date starTime, Map<String, Map<String, Double>> valueCacheMap,
...
String valueCacheKey = "[" + indicatorSetModel.getMoTypeName() + "]." + "["
        + indicatorSetModel.getIndicatorSetName() + "]." + "[" + indicatorModel.getIndicatorName() + "]";
```
